 ...
 */
@AndroidEntryPoint
internal class FineEditDocumentFragment : BaseFragment<PsFragmentFormBinding, FineEditDocumentViewModel>() {
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        requireActivity().menuInflater.inflate(R.menu.ps_menu_delete, menu)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when(item.itemId) {
            R.id.action_delete -> {
                SimpleDialog.show(fragment = this,
                    titleId = R.string.ps_remove_doc_title,
                    textId = R.string.ps_remove_doc_text,
                ) {
                    viewModel.onActionDelete()
                }
                return true
            }
